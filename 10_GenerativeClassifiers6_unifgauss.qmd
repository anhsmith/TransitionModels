...
The PDF of the uniform-Gaussian was modified from the PDF uniform and two Guassians in Lau & Krumscheid [-@lau_plateau_2022].
...
```{r}
ggfit <- data.frame(x=data_list$x,y=data_list$y) |> 
  ggplot() +
  geom_histogram(
  ...
  geom_line(
    data = fit |> 
      as_draws_df() |> 
      thin_draws(thin = 40) |> 
      expand_grid(x = seq(from=850, to=1650, length=10000)) |> 
      rowwise() |> 
      mutate(
...
```{r}
ggfitnice <- ggfit + theme_void() + theme(legend.position = "none")
```

```{r}
ggsave("plotnice.png", ggfitnice, width=6, height=4)
```
