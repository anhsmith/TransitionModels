data {
  ...
  real L;   // minimum truncation point
  real U;   // maximum truncation point
  real<lower=L, upper=U> x[N]; // predictor values
...
model {
  ...
  for (i in 1:N) {
    if (m[i] == 0) {
      x[i] ~ normal(mu0[pop[i]], sigma) T[L, U];
    } else {
      x[i] ~ normal(mu1[pop[i]], sigma) T[L, U];
    }
  }
}
